	...
	public class PagerIndicatorTabs : Grid
	{
		...
		void CreateTabs()
		{
			...
			foreach(var item in ItemsSource)
			{
                var sourceImage = "";
                if (i == 0)
                    sourceImage = "delivery_complete.png";
                else
                {
                      sourceImage = "completed_tasks.png";
                    }
                i++;
				...
				Device.OnPlatform(
					iOS: () =>
					{
						tab.Children.Add(new Image { Source =  sourceImage, HeightRequest = 20 });
						tab.Children.Add(new Label { Text = "Tab " + (index + 1), FontSize = 11 });
					},					
					Android: () =>
					{
                    tab.Children.Add(new Image { Source = sourceImage, HeightRequest = 25,WidthRequest=100 });
					}
				);
