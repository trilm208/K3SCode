...
namespace DataAccess
{
	class Config
	{
        ...
        static string  DefaultPath = Xamarin.Forms.DependencyService.Get<IStringExtensions>().GetCurrentAppDomain()+ "Config.xml";
		...
		private static   Config Load(string path)
		{
			...
			try
			{
                ...
                var resultLoadData = Xamarin.Forms.DependencyService.Get<IXmlReader>().LoadFromXML(path);

                foreach(var item in resultLoadData)
                    {
